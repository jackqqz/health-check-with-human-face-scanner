...
% 
function wave2Pic(wave, numFrames, dst_path)
%wave2Pic  Creates a simple "wave.png" from the wave signal.

if ~exist(dst_path, 'dir')
    mkdir(dst_path);
end

img_path = fullfile(dst_path, 'wave.png');
waveSize = size(wave);
w_x = linspace(0,100,waveSize(1));
d_x = linspace(0,100,numFrames);
...
for c = 1:3
    ...
    wave_final(1,:,c) = (temp - min(temp)) / denom * 255;
end

img = wave_final;
imwrite(uint8(img), img_path);
disp(['>> wave.png saved to: ', img_path]);
end

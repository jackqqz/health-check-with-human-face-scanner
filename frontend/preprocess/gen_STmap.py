...
def poly2mask(_lmk, _shape, val=1, b_val=0):
    if b_val == 0:
        hull_mask = np.zeros(_shape[0:2], dtype=np.uint8)
    else:
        hull_mask = np.ones(_shape[0:2], dtype=np.uint8)

    cv2.fillPoly(hull_mask, [np.array(_lmk, dtype=np.int32)], color=val)
    return hull_mask
